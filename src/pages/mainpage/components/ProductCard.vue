...
export default {
  ...
  methods: {
    ...mapMutations("device_jcsb", ["set_zx", "set_ly"]),
    ...
    update_product_card_info() {
        ...
        .then((result) => {
          ...
          zx_list.forEach((device) => {
            if (device.status == 1) {
              ...
              let temp_last_time = new Date(device.last_time);
              console.log(now - temp_last_time);
              if (now - temp_last_time < this.dataSource[0].offline_timespan) {
                online++;
          ...
          this.set_zx({ all: temp_all });
          this.set_zx({ all_device: zx_list });
          this.set_zx({ un_activation: un_activation });
          this.set_zx({ un_activation_device: un_activation_device });
          this.set_zx({ online: online });
          this.set_zx({ online_device: online_device });
          this.set_zx({ offline: offline });
          this.set_zx({ offline_device: offline_device });
        })
        .catch((err) => {
          console.log(err);
        });
      get_user_devicelist_ly({
        ...
        .then((result) => {
          ...
          this.set_ly({ all: temp_all });
          this.set_ly({ all_device: zx_list });
          this.set_ly({ un_activation: un_activation });
          this.set_ly({ un_activation_device: un_activation_device });
          this.set_ly({ online: online });
          this.set_ly({ online_device: online_device });
          this.set_ly({ offline: offline });
          this.set_ly({ offline_device: offline_device });
          this.loading = false;
        })
        .catch((err) => {
          console.log(err);
